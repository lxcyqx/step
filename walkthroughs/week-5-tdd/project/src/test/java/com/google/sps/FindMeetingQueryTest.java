...
public final class FindMeetingQueryTest {
  ...
  public void everyAttendeeIsConsideredWithOptional() {
    ...
    @Test
  public void everyAttendeeIsConsideredWithoutOptional() {
    // Have each person have different events. We should see two options because each person has
    // split the restricted times. Optional attendee has whole day event so return available times /// for mandatory attendees.
    //
    // Events  :|--------------C---------------|       
    //                 |--A--|     |--B--|
    // Day     : |-----------------------------|
  ...
  public void justEnoughRoomWithoutOptional() {
    // Have one person, but make it so that there is just enough room at one point in the day to
    // have the meeting. There is second optional person but they are ignored since their schedule
  ...
  @Test
  public void noMandatoryJustOptional(){
    // No mandatory attendees, just two optional attendees with several gaps in schedule.
    //
    // Events  : |--A--|  |--B--|  |-A-|
    // Day     : |---------------------|
    // Options :       |--|     |--|
    ...
    request.addOptionalAttendee(PERSON_B);
    Collection<TimeRange> actual = query.query(events, request);
    Collection<TimeRange> expected = Arrays.asList(TimeRange.fromStartEnd(TIME_0830AM, TIME_0900AM, false), TimeRange.fromStartEnd(TIME_0930AM, TIME_1100AM, false));
    Assert.assertEquals(expected, actual);
  }

  @Test
  public void noMandatoryJustOptionalNoGaps() {
    // No mandatory attendees, just two optional attendees with no gaps in schedule.
    //
    // Events  : |--A-----|--B---------|
    // Day     : |---------------------|
    // Options :       
